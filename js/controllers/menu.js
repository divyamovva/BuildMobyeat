...
.config(function() {
		window.Stripe.setPublishableKey('pk_test_LCZAeyeOu4IFCNkOSiShJkhW');
	})
...
$scope.getTotal = function ( cart ) {
       ...
       angular.forEach(cart , function (cartitem) {
             ...
             for(var subcat in cartitem.sub_item ) {
              ...
              quantity=quantity+Number(cartitem.qty);
       });
       $scope.itemsCount=quantity;
...
.controller("ItemCtrl" , function ($scope , $location , $rootScope ,mobyeatUrl,$ionicPopup, $stateParams , $state)  {
...
$scope.getAddr=function()   
{
	...
	var user_address = JSON.parse(localStorage.getItem("default_user_address") || "{}");
       if(user_address.street || user_address.city){
