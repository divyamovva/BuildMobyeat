    ...
    $rootScope.$on('$stateChangeSuccess', 
	  function(event, toState, toParams, fromState, fromParams){
	      if($state.current.name == "profile") $scope.profilepage = true;
                       ...
                       if(toState.name=='menucard' ||toState.name=='cart' || toState.name == "checkout" || toState.name == "orderitem" ){
                          $scope.backbtn=true;
...
.controller("RestoCtrl" , function ( $scope ,custom, $rootScope ,$ionicLoading,  mobyeat , $stateParams , $location , getMerchants , rest , $ionicModal , $ionicPopup ) {
  ...
  $scope.viewMap = function (restostreet,restocity) {
   ...
   $scope.getMenu = function ( merchant_id , menu_id ,restaurant_slug,menu_name) {
  
   	      $scope.slug=restaurant_slug
       var cart = JSON.parse(localStorage.getItem('cart')) || []; 
                if($scope.preMenu){
             if($scope.preMenu.id!=menu_id && cart.length!=0){
             	var confirmPopup = $ionicPopup.confirm({
				     title: 'Confirm',
				     template: 'You have pending order with the menu '+$scope.preMenu.name
                          +'Do you want to abandon and restart a new order with'+menu_name	
                         			   });
				   confirmPopup.then(function(res) {
                     ...
                     $location.url('menucard/'+merchant_id+'/'+menu_id+'/'+Date.now());
                       ...
                       $location.url('menucard/'+merchant_id+'/'+$scope.preMenu.id+'/'+Date.now());
				     }
				   });
...
.controller( 'OrdersCtrl', function($scope, $ionicLoading, $stateParams , orders , $location) {
  $scope.setInits = function () {
  	       	...
  	       	$scope.orders = orders;
                  ...
                  $scope.orders[i].delivery_time=$scope.orders[i].delivery_time.split('-');
                  $scope.orders[i].delivery_time=$scope.orders[i].delivery_time[1];	       		
  	       		}
  }
